...
import com.snehil.prolink.connection_service.config.KafkaTopicConfig;
...
public class ConnectionRequestService {
    ...
    public ConnectionRequestDto sendRequest(Long receiverId) {
        ...
        publishConnectionRequestSent(request);
    ...
    public ConnectionRequestDto acceptRequest(Long requestId) {
        ...
        publishConnectionRequestAccepted(request);
        return toDto(request);
    }
    ...
    public ConnectionRequestDto rejectRequest(Long requestId) {
        ...
        publishConnectionRequestRejected(request);
        return toDto(request);
    }
    ...
    private void publishConnectionRequestSent(ConnectionRequest request) {
        try {
            ConnectionRequestSentEvent event = ConnectionRequestSentEvent.builder()
                    ...
                    .build();
            kafkaTemplate.send(KafkaTopicConfig.CONNECTION_REQUEST_SENT_TOPIC, request.getReceiverId(), event);
        } catch (Exception e) {
            log.warn("Failed to publish connection request sent event: {}", e.getMessage());
        }
    }

    private void publishConnectionRequestAccepted(ConnectionRequest request) {
        try {
            ConnectionRequestAcceptedEvent event = ConnectionRequestAcceptedEvent.builder()
                    ...
                    .receiverId(request.getReceiverId())
                    .build();
            kafkaTemplate.send(KafkaTopicConfig.CONNECTION_REQUEST_ACCEPTED_TOPIC, request.getSenderId(), event);
        } catch (Exception e) {
            log.warn("Failed to publish connection request accepted event: {}", e.getMessage());
        }
    }

    private void publishConnectionRequestRejected(ConnectionRequest request) {
        try {
            ConnectionRequestRejectedEvent event = ConnectionRequestRejectedEvent.builder()
                    .requestId(request.getId())
                    .senderId(request.getSenderId())
                    .receiverId(request.getReceiverId())
                    .build();
            kafkaTemplate.send(KafkaTopicConfig.CONNECTION_REQUEST_REJECTED_TOPIC, request.getSenderId(), event);
        } catch (Exception e) {
            log.warn("Failed to publish connection request rejected event: {}", e.getMessage());
        }
    }
